...
export default function Sidebar({ open, onClose }) {
  ...
  const content = (
    <div className="flex flex-col h-full bg-[#0D0D0D] border-r border-[#262626] w-64">
      {/* Logo */}
      <div className="px-6 py-5 border-b border-[#262626] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <img
            src="/logo-suplementostuc.png"
            alt="Logo"
            className="h-8 w-auto"
            onError={(e) => (e.target.style.display = "none")}
          />
          <div>
            <p className="text-xs font-bold text-[#FAFAFA] leading-tight">SUPLEMENTOS</p>
            <p className="text-xs text-[#FF6B1A] font-bold leading-tight">TUCUMÁN</p>
          </div>
        </div>
        {onClose && (
          <button onClick={onClose} className="lg:hidden text-[#A1A1AA] hover:text-[#FAFAFA]">
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <div className="px-3 py-4 border-t border-[#262626]">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-[#A1A1AA] hover:text-[#EF4444] hover:bg-[#EF4444]/5 transition-colors"
        >
          <LogOut size={18} />
          Cerrar sesión
        </button>
